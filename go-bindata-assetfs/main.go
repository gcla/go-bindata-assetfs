...
func main() {
	...
	if err != nil {
		...
		fmt.Println("Maybe you need: go get github.com/gcla/go-bindata-assetfs/...")
		os.Exit(1)
	}
	...
	for line, isPrefix, err := r.ReadLine(); err == nil; line, isPrefix, err = r.ReadLine() {
		...
		if !done && !isPrefix && bytes.HasPrefix(line, []byte("import (")) {
			if debug {
				...
				fmt.Fprintln(out, "\t\"github.com/gcla/go-bindata-assetfs\"")
			}
			done = true
		}
	}
	if debug {
		fmt.Fprintln(out, `
func MakeAssetFS() http.FileSystem {
	...
	panic("unreachable")
		...
		fmt.Fprintln(out, `
func MakeAssetFS() *assetfs.AssetFS {
